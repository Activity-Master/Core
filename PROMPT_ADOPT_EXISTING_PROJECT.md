...
- Repository URL / local path: https://github.com/Activity-Master/Core.git
- Org and project name: ActivityMaster / Core
- Short description: Implementation project for the Activity Master client built with reactive Java and Hibernate that enforces a canonical database model aligned with the domain boundaries.
...
- Detected/Chosen tech topics (tick):
  ...
  - Backend Reactive:
    ...
    - Data access libraries:
      - [X] EntityAssist — rules/generative/data/entityassist/README.md
    - Security (Reactive):
...
- Testing: Tests must execute as the full suite; every run needs to provision a new "enterprise" via a complete install (see `src/test/java/com/guicedee/activitymaster/tests/TestActivityMasterManageAddresses.java` and similar cases that rely on Testcontainers). Standalone tests are not valid because the system must always be configured before use.
