...
BEGIN
    ...
    FOR v_schema_name, v_table_name, v_column_name IN
        ...
        WHERE c.data_type = 'character varying'
          ...
          AND c.table_schema NOT IN ('postgres', 'pg_catalog', 'information_schema')
            and c.column_name NOT IN ('value','enterprisedesc','enterprisename','involvedpartytypedesc','resourceitemtypedesc','securitytokenfriendlydescription','securitytokenfriendlyname')
        LOOP
            BEGIN
                -- Process foreign keys
                FOR foreign_key_name, fk_table_name, fk_schema_name, fk_column_name IN
...
CREATE OR REPLACE FUNCTION public.update_varchar_to_uuid_orig()
    RETURNS void AS
$$
DECLARE
    v_table_name       TEXT;
...
BEGIN
    ...
    FOR v_schema_name, v_table_name, v_column_name IN
        ...
        WHERE c.data_type = 'character varying'
          AND c.character_maximum_length = 255
          AND c.column_name = 'originalsourcesystemid'
          AND c.table_schema NOT IN ('postgres', 'pg_catalog', 'information_schema')
        LOOP
            BEGIN
                -- Process foreign keys
...
SELECT public.update_varchar_to_uuid_orig();
...
BEGIN
    ...
    FOR v_schema_name, v_table_name, v_column_name IN
        ...
        WHERE c.data_type = 'character varying'
          AND c.character_maximum_length = 255
          AND c.column_name = 'originalsourcesystemuniqueid'
          AND c.table_schema NOT IN ('postgres', 'pg_catalog', 'information_schema')
        LOOP
            BEGIN
                -- Process foreign keys
...
SELECT DISTINCT exet.value
  ...
  WHERE et.eventtypedesc::text = 'BarcodeReceived'::text