...
public class EventsBaseSetup implements ISystemUpdate
{
	...
	{
		log.info("Starting sequential creation of event classifications with session: {}", session.hashCode());
	...
	// =============================================================================================
	// Stateless twins — mirror the managed seeding but run on a Mutiny.StatelessSession. Required by the
	// stateless install loop (EnterpriseService.processUpdates(Mutiny.StatelessSession, …)); without them
	// the base default throws and the failure is swallowed, leaving the event classifications unseeded.
	// =============================================================================================

	@Override
	public Uni<Boolean> update(Mutiny.StatelessSession session, IEnterprise<?,?> enterprise)
	{
		log.info("Starting sequential creation of event classifications (stateless)");
		logProgress("Events System", "Loading Base Events...", 1);
	...
	@SuppressWarnings("WeakerAccess")
	Uni<Void> createEventInvolvedPartyDefaultClassifications(Mutiny.StatelessSession session, IEnterprise<?,?> enterprise)
	{
		log.info("Creating involved party event classifications (stateless)");
		ISystemsService<?> systemsService = IGuiceContext.get(ISystemsService.class);
		return systemsService.findSystem(session, enterprise, ActivityMasterSystemName)
			.chain(activityMasterSystem -> service.create(session, EventInvolvedPartiesClassifications.InvolvedPartyEvents, activityMasterSystem)
				.chain(baseClassification -> service.create(session, EventInvolvedPartiesClassifications.PerformedBy, activityMasterSystem, EventInvolvedPartiesClassifications.InvolvedPartyEvents)
					.chain(() -> service.create(session, EventInvolvedPartiesClassifications.OnBehalfOf, activityMasterSystem, EventInvolvedPartiesClassifications.InvolvedPartyEvents))
					.chain(() -> service.create(session, EventInvolvedPartiesClassifications.For, activityMasterSystem, EventInvolvedPartiesClassifications.InvolvedPartyEvents))
					.chain(() -> service.create(session, EventInvolvedPartiesClassifications.OwnedBy, activityMasterSystem, EventInvolvedPartiesClassifications.InvolvedPartyEvents))
					.chain(() -> service.create(session, EventInvolvedPartiesClassifications.Created, activityMasterSystem, EventInvolvedPartiesClassifications.InvolvedPartyEvents))
					...
					.onFailure().invoke(error -> log.error("❌ Error creating involved party event classifications (stateless): {}", error.getMessage(), error))
					.replaceWithVoid()));
	}

	@SuppressWarnings("WeakerAccess")
	Uni<Void> createEventAddressDefaultClassifications(Mutiny.StatelessSession session, IEnterprise<?,?> enterprise)
	{
		log.info("Creating address event classifications (stateless)");
		ISystemsService<?> systemsService = IGuiceContext.get(ISystemsService.class);
		return systemsService.findSystem(session, enterprise, ActivityMasterSystemName)
			.chain(activityMasterSystem -> service.create(session, EventAddressClassifications.AddressEvents, activityMasterSystem)
				.chain(baseClassification -> service.create(session, EventAddressClassifications.SignedAt, activityMasterSystem, EventAddressClassifications.AddressEvents)
					.chain(() -> service.create(session, EventAddressClassifications.OccurredAt, activityMasterSystem, EventAddressClassifications.AddressEvents))
					.chain(() -> service.create(session, EventAddressClassifications.RemoteAddress, activityMasterSystem, EventAddressClassifications.AddressEvents))
					.chain(() -> service.create(session, EventAddressClassifications.LocalAddress, activityMasterSystem, EventAddressClassifications.AddressEvents))
					.chain(() -> service.create(session, EventAddressClassifications.PhonedNumber, activityMasterSystem, EventAddressClassifications.AddressEvents))
					...
					.onFailure().invoke(error -> log.error("❌ Error creating address event classifications (stateless): {}", error.getMessage(), error))
					.replaceWithVoid()));
	}

	@SuppressWarnings("WeakerAccess")
	Uni<Void> createEventArrangementDefaultClassifications(Mutiny.StatelessSession session, IEnterprise<?,?> enterprise)
	{
		log.info("Creating arrangement event classifications (stateless)");
		ISystemsService<?> systemsService = IGuiceContext.get(ISystemsService.class);
		return systemsService.findSystem(session, enterprise, ActivityMasterSystemName)
			.chain(activityMasterSystem -> service.create(session, EventArrangementClassifications.ArrangementEvents, activityMasterSystem)
				.chain(baseClassification -> service.create(session, EventArrangementClassifications.Started, activityMasterSystem, EventArrangementClassifications.ArrangementEvents)
					.chain(() -> service.create(session, EventArrangementClassifications.Concluded, activityMasterSystem, EventArrangementClassifications.ArrangementEvents))
					.chain(() -> service.create(session, EventArrangementClassifications.AffectedThe, activityMasterSystem, EventArrangementClassifications.ArrangementEvents))
					.chain(() -> service.create(session, EventArrangementClassifications.RestartedThe, activityMasterSystem, EventArrangementClassifications.ArrangementEvents))
					.chain(() -> service.create(session, EventArrangementClassifications.SkippedThe, activityMasterSystem, EventArrangementClassifications.ArrangementEvents))
					...
					.onFailure().invoke(error -> log.error("❌ Error creating arrangement event classifications (stateless): {}", error.getMessage(), error))
					.replaceWithVoid()));
	}

	@SuppressWarnings("WeakerAccess")
	Uni<Void> createEventEventTypesClassifications(Mutiny.StatelessSession session, IEnterprise<?,?> enterprise)
	{
		log.info("Creating event type classifications (stateless)");
		ISystemsService<?> systemsService = IGuiceContext.get(ISystemsService.class);
		return systemsService.findSystem(session, enterprise, ActivityMasterSystemName)
			.chain(activityMasterSystem -> service.create(session, EventTypeClassifications.TypeOfEvents, activityMasterSystem)
				.chain(baseClassification -> service.create(session, EventTypeClassifications.HasTheType, activityMasterSystem, EventTypeClassifications.TypeOfEvents)
					.chain(() -> service.create(session, EventTypeClassifications.CanBeIdentifiedBy, activityMasterSystem, EventTypeClassifications.TypeOfEvents))
					.invoke(() -> logProgress("Events System", "Loaded Event Event Types Classifications...", 1))
					.onFailure().invoke(error -> log.error("❌ Error creating event type classifications (stateless): {}", error.getMessage(), error))
					.replaceWithVoid()));
	}

	@SuppressWarnings("WeakerAccess")
	Uni<Void> createEventProductsDefaultClassifications(Mutiny.StatelessSession session, IEnterprise<?,?> enterprise)
	{
		log.info("Creating product event classifications (stateless)");
		ISystemsService<?> systemsService = IGuiceContext.get(ISystemsService.class);
		return systemsService.findSystem(session, enterprise, ActivityMasterSystemName)
			.chain(activityMasterSystem -> service.create(session, EventProductClassifications.ProductEvent, activityMasterSystem)
				.chain(baseClassification -> service.create(session, EventProductClassifications.ShowedInterestIn, activityMasterSystem, EventProductClassifications.ProductEvent)
					.chain(() -> service.create(session, EventProductClassifications.Bought, activityMasterSystem, EventProductClassifications.ProductEvent))
					.chain(() -> service.create(session, EventProductClassifications.Sold, activityMasterSystem, EventProductClassifications.ProductEvent))
					.chain(() -> service.create(session, EventProductClassifications.MadeBidFor, activityMasterSystem, EventProductClassifications.ProductEvent))
					.chain(() -> service.create(session, EventProductClassifications.ChangedBidFor, activityMasterSystem, EventProductClassifications.ProductEvent))
					...
					.onFailure().invoke(error -> log.error("❌ Error creating product event classifications (stateless): {}", error.getMessage(), error))
					.replaceWithVoid()));
	}

	@SuppressWarnings("WeakerAccess")
	Uni<Void> createEventResourceItemDefaultClassifications(Mutiny.StatelessSession session, IEnterprise<?,?> enterprise)
	{
		log.info("Creating resource item event classifications (stateless)");
		ISystemsService<?> systemsService = IGuiceContext.get(ISystemsService.class);
		return systemsService.findSystem(session, enterprise, ActivityMasterSystemName)
			.chain(activityMasterSystem -> service.create(session, EventResourceItemClassifications.ResourceItemEvent, activityMasterSystem)
				.chain(baseClassification -> service.create(session, EventResourceItemClassifications.AddedResourceItem, activityMasterSystem, EventResourceItemClassifications.ResourceItemEvent)
					.chain(() -> service.create(session, EventResourceItemClassifications.ChangedTheResourceItem, activityMasterSystem, EventResourceItemClassifications.ResourceItemEvent))
					.chain(() -> service.create(session, EventResourceItemClassifications.UpdatedTheResourceItem, activityMasterSystem, EventResourceItemClassifications.ResourceItemEvent))
					.chain(() -> service.create(session, EventResourceItemClassifications.RemovedTheResourceItem, activityMasterSystem, EventResourceItemClassifications.ResourceItemEvent))
					.chain(() -> service.create(session, EventResourceItemClassifications.RegisteredTheResourceItem, activityMasterSystem, EventResourceItemClassifications.ResourceItemEvent))
					...
					.onFailure().invoke(error -> log.error("❌ Error creating resource item event classifications (stateless): {}", error.getMessage(), error))
					.replaceWithVoid()));
	}
}