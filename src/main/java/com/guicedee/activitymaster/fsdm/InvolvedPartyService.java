...
public class InvolvedPartyService implements IInvolvedPartyService<InvolvedPartyService> {
    ...
    private final com.guicedee.activitymaster.fsdm.util.BoundedLruCache<String, java.util.UUID> involvedPartyKeyToId = new com.guicedee.activitymaster.fsdm.util.BoundedLruCache<>(
            PARTY_MAX);
    private final com.guicedee.activitymaster.fsdm.util.BoundedLruCache<java.util.UUID, IInvolvedParty<?, ?>> involvedPartyIdToStatelessEntity = new com.guicedee.activitymaster.fsdm.util.BoundedLruCache<>(
            PARTY_MAX);
    private final com.guicedee.activitymaster.fsdm.util.BoundedLruCache<String, java.util.UUID> involvedPartyTypeKeyToId = new com.guicedee.activitymaster.fsdm.util.BoundedLruCache<>(
            TYPE_MAX);
    private final com.guicedee.activitymaster.fsdm.util.BoundedLruCache<String, java.util.UUID> involvedPartyNameTypeKeyToId = new com.guicedee.activitymaster.fsdm.util.BoundedLruCache<>(
            NAME_TYPE_MAX);
    ...
    static {
        log.info(
                "InvolvedParty caches configured: totalMb={}, entryEstBytes={}, splits=[party:{}, type:{}, nameType:{}, identType:{}], maxEntries=[party:{}, type:{}, nameType:{}, identType:{}]",
                TOTAL_MB,
                ENTRY_EST_BYTES,
                SPLITS[0],
                SPLITS[1],
    ...
    private Uni<IInvolvedParty<?, ?>> getStatelessInvolvedPartyById(Mutiny.StatelessSession session, UUID id) {
        ...
        return loadStatelessInvolvedPartyById(session, id).invoke(this::cacheStatelessInvolvedParty);
    }

    @SuppressWarnings({"unchecked", "rawtypes"})
    private Uni<IInvolvedParty<?, ?>> loadStatelessInvolvedPartyById(Mutiny.StatelessSession session, UUID id) {
        return (Uni) new InvolvedParty().builder(session).find(id).get().onFailure(NoResultException.class)
                                        .invoke(e -> log.warn(
                                                "InvolvedParty with UUID '{}' could not be found (stateless)",
                                                id)).onItem().ifNull()
    ...
    public Uni<IInvolvedPartyIdentificationType<?, ?>> findInvolvedPartyIdentificationType(Mutiny.StatelessSession session,
                                                                                           @CacheKey String idType,
                                                                                           @CacheKey ISystems<?, ?> system,
                                                                                           UUID... identityToken) {
        if (idType == null) {
            return Uni.createFrom()
                      .failure(new NoResultException("InvolvedPartyIdentificationType name must not be null"));
        }
        var enterprise = system.getEnterprise();
        return new InvolvedPartyIdentificationType().builder(session).withName(idType).inActiveRange().inDateRange()
    ...
    public Uni<IInvolvedPartyNameType<?, ?>> findInvolvedPartyNameType(Mutiny.StatelessSession session,
                                                                       @CacheKey String nameType,
                                                                       @CacheKey ISystems<?, ?> system,
                                                                       UUID... identityToken) {
        if (nameType == null) {
        ...
        var enterprise = system.getEnterprise();
        return new InvolvedPartyNameType().builder(session).withName(nameType).inActiveRange().inDateRange()
                                          .withEnterprise(enterprise).selectColumn(InvolvedPartyNameType_.id)
                                          .selectColumn(InvolvedPartyNameType_.name)
                                          .selectColumn(InvolvedPartyNameType_.description).get(Object[].class)
                                          .map(row -> {
                                              InvolvedPartyNameType prepped = new InvolvedPartyNameType((UUID) row[0],
                                                                                                        (String) row[1],
                                                                                                        (String) row[2]);
                                              prepped.setEnterpriseID(enterprise);
                                              prepped.setFake(false);
    ...
    public Uni<IInvolvedPartyType<?, ?>> findType(Mutiny.StatelessSession session,
                                                  @CacheKey String nameType,
                                                  @CacheKey ISystems<?, ?> system,
                                                  UUID... identityToken) {
        if (nameType == null) {
        ...
        var enterprise = system.getEnterprise();
        return new InvolvedPartyType().builder(session).withName(nameType).inActiveRange().inDateRange()
                                      .withEnterprise(enterprise).selectColumn(InvolvedPartyType_.id)
                                      .selectColumn(InvolvedPartyType_.name)
                                      .selectColumn(InvolvedPartyType_.description).get(Object[].class).map(row -> {
    ...
    @Override
    public Uni<IInvolvedPartyIdentificationType<?, ?>> createIdentificationType(Mutiny.StatelessSession session,
                                                                                ISystems<?, ?> system,
                                                                                String name,
                                                                                String description,
                                                                                UUID... identityToken) {
        ...
        return findInvolvedPartyIdentificationType(session, name, system, identityToken).onFailure()
                                                                                        .recoverWithUni(err -> {
                                                                                            ...
                                                                                            ISecurityTokenService<?> sts = IGuiceContext.get(
                                                                                                    ISecurityTokenService.class);
                                                                                            return acService
                                                                                                    .getActiveFlag(
                                                                                                            session,
                                                                                                            enterprise,
                                                                                                        ...
                                                                                                        return xr
                                                                                                                ...
                                                                                                                .chain(persisted -> sts
                                                                                                                        .resolveDefaultGroupFolderTokens(
                                                                                                                                ...
                                                                                                                                system,
                                                                                                                                identityToken)
                                                                                                                        .chain(tokens -> xr.createDefaultSecurity(
                                                                                                                                session,
                                                                                                                                system,
                                                                                                                                enterprise,
                                                                                                                                activeFlag,
                                                                                                                        ...
                                                                                                                        .recoverWithItem(
                                                                                                                                0L)
                                                                                                                        .replaceWith(
                                                                                                                                (IInvolvedPartyIdentificationType<?, ?>) xr));
                                                                                                    });
                                                                                        });
    }

    @Override
    public Uni<IInvolvedPartyNameType<?, ?>> createNameType(Mutiny.StatelessSession session,
                                                            String name,
                                                            String description,
                                                            ISystems<?, ?> system,
                                                            UUID... identityToken) {
        var enterprise = system.getEnterprise();
        return findInvolvedPartyNameType(session, name, system, identityToken).onFailure().recoverWithUni(err -> {
                ...
                return xr.builder(session).persist(xr)
                         .chain(persisted -> sts.resolveDefaultGroupFolderTokens(session, system, identityToken)
                                                .chain(tokens -> xr.createDefaultSecurity(session,
                                                                                          ...
                                                                                          activeFlag,
                                                                                          tokens)).onFailure()
                                                .recoverWithItem(0L).replaceWith((IInvolvedPartyNameType<?, ?>) xr));
            });
        });
    }

    @Override
    public Uni<IInvolvedPartyType<?, ?>> createType(Mutiny.StatelessSession session,
                                                    ISystems<?, ?> system,
                                                    String name,
                                                    String description,
                                                    UUID... identityToken) {
        var enterprise = system.getEnterprise();
        return findType(session, name, system, identityToken).onFailure().recoverWithUni(err -> {
                ...
                return xr.builder(session).persist(xr)
                         .chain(persisted -> sts.resolveDefaultGroupFolderTokens(session, system, identityToken)
                                                .chain(tokens -> xr.createDefaultSecurity(session,
                                                                                          ...
                                                                                          activeFlag,
                                                                                          tokens)).onFailure()
                                                .recoverWithItem(0L).replaceWith((IInvolvedPartyType<?, ?>) xr));
            });
        });
    }
    ...
    @Override
    public Uni<IInvolvedParty<?, ?>> create(Mutiny.StatelessSession session,
                                            ISystems<?, ?> system,
     ...
     */
    @Override
    public Uni<IInvolvedParty<?, ?>> createScopeRestricted(Mutiny.StatelessSession session,
                                                           ISystems<?, ?> system,
                                                           UUID key,
                                                           Pair<String, String> idTypes,
                                                           boolean isOrganic,
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    private Uni<IInvolvedParty<?, ?>> createStateless(Mutiny.StatelessSession session,
                                                      ISystems<?, ?> system,
                                                      UUID key,
                                                      Pair<String, String> idTypes,
                                                      boolean isOrganic,
        ...
        return acService.getActiveFlag(session, enterprise, identityToken).chain(activeFlag -> {
            ...
            return session.insert(ip).chain(() -> sts.resolveDefaultGroupFolderTokens(session, system, identityToken)
                                                     .chain(tokens -> restricted ? ip.createScopeRestrictedSecurity(
                                                             session,
                                                             system,
                                                             enterprise,
                                                             activeFlag,
                                                             tokens,
                                                             scopeToken,
                                                             identityToken) : ip.createDefaultSecurity(session,
                                                                                                       system,
                                                                                                       enterprise,
                                                                                                       activeFlag,
                                                                                                       tokens,
                          ...
                          .chain(() -> findInvolvedPartyIdentificationType(session,
                                                                           idTypes.getKey(),
                                                                           system,
                                                                           identityToken).chain(ipIdType -> ip.addOrReuseInvolvedPartyIdentificationType(
                                  session,
                                  NoClassification.toString(),
                                  ipIdType,
                                  idTypes.getValue(),
                                  system,
                                  identityToken))).chain(() -> setupInvolvedPartyOrganicStatusStateless(session,
                                                                                                        isOrganic,
                                                                                                        ip,
                                                                                                        system,
                                                                                                        scopeToken,
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    private Uni<Void> setupInvolvedPartyOrganicStatusStateless(Mutiny.StatelessSession session,
                                                               boolean isOrganic,
                                                               InvolvedParty ip,
                                                               ISystems<?, ?> system,
                                                               ISecurityToken<?, ?> scopeToken,
        ...
        return acService.getActiveFlag(session, enterprise, identityToken).chain(activeFlag -> {
            ...
            return session.insert(record)
                          .chain(() -> sts.resolveDefaultGroupFolderTokens(session, system, identityToken)
                                          .chain(tokens -> restricted ? ((IWarehouseCoreTable) record).createScopeRestrictedSecurity(
                                                  session,
                                                  system,
                                                  enterprise,
                                                  activeFlag,
                                                  tokens,
                                                  scopeToken,
                                                  identityToken) : ((IWarehouseCoreTable) record).createDefaultSecurity(
                                                  session,
                                                  system,
                                                  enterprise,
                                                  activeFlag,
                                 ...
                                 .chain(activityMasterSystem -> {
                                     return persisted
                                             ...
                                             .onItem().invoke(result -> log.debug(
                                                     "Security setup completed successfully for organic type {}",
                                                     persisted.getName()))
            ...
            return securityFn.apply(persisted).chain(() -> findInvolvedPartyIdentificationType(session,
                    ...
                    .chain(involvedPartyIdentificationType -> {
                        ...
                        return setupInvolvedPartyOrganicStatus(session,
                                                               ...
                                                               securityFn,
                                                               system,
                                                               identityToken).chain(() -> Uni.createFrom()
        ...
        if (isOrganic) {
            ...
            return acService.getActiveFlag(session, enterprise, identityToken).chain(activeFlag -> {
                ...
                return securityFn.apply(persisted).onItem().invoke(result -> log.debug(
                        "Security setup completed successfully for organic involved party {}",
                        persisted.getId())).replaceWithVoid();
            });
        } else {
            InvolvedPartyNonOrganic ipo = new InvolvedPartyNonOrganic();
            ipo.setInvolvedParty((InvolvedParty) ip);
            ...
            return acService.getActiveFlag(session, enterprise, identityToken).chain(activeFlag -> {
                ...
                // Handle security setup sequentially
                return securityFn.apply(persisted).onItem().invoke(result -> log.debug(
                        "Security setup completed successfully for non-organic involved party {}",
                        persisted.getId())).replaceWithVoid();
            });
        }
    }
    ...
    private Uni<IInvolvedParty<?, ?>> proceedFindByTokenColdPath(Mutiny.StatelessSession session,
        ...
        return findInvolvedPartyIdentificationType(session,
                                                   ...
                                                   ((SecurityToken) token).getSystemID(),
            ...
            return idType.builder(session)
                         ...
                         .onFailure(NoResultException.class).invoke(e -> log.warn(
                            "InvolvedParty by token '{}' could not be found",
                            token.getSecurityToken())).onItem()
    ...
    public Uni<IInvolvedPartyNameType<?, ?>> findNameType(Mutiny.StatelessSession session, UUID uuid) {
        ...
        return (Uni) new InvolvedPartyNameType().builder(session).find(uuid).get().onFailure(NoResultException.class)
                                                .invoke(e -> log.warn(
                                                        "InvolvedPartyNameType with UUID '{}' could not be found (stateless)",
                                                        uuid)).onItem().ifNull()
                                                .failWith(() -> new InvolvedPartyException(
                                                        "The InvolvedPartyNameType does not exist - " + uuid));
    }

    @Override
    @SuppressWarnings({"unchecked", "rawtypes"})
    public Uni<IInvolvedPartyIdentificationType<?, ?>> findIdentificationType(Mutiny.StatelessSession session,
                                                                              UUID uuid) {
        ...
        return (Uni) new InvolvedPartyIdentificationType().builder(session).find(uuid).get()
                                                          .onFailure(NoResultException.class).invoke(e -> log.warn(
                        "InvolvedPartyIdentificationType with UUID '{}' could not be found (stateless)",
                        uuid));
    }
    ...
    @Override
    @SuppressWarnings({"unchecked", "rawtypes"})
    public Uni<IInvolvedParty<?, ?>> findByResourceItem(Mutiny.StatelessSession session,
                                                        IResourceItem<?, ?> idType,
                                                        String value,
                                                        ISystems<?, ?> system,
                                                        UUID... identityToken) {
        log.debug("Finding InvolvedParty by ResourceItem (stateless): value={}", value);
        return (Uni) new InvolvedPartyXResourceItem().builder(session).canRead(system, identityToken).inActiveRange()
                                                     .inDateRange().findLink(null, (ResourceItem) idType, value)
                                                     .setReturnFirst(true).get().onFailure(NoResultException.class)
                                                     .invoke(e -> log.warn(
                                                             "InvolvedParty by ResourceItem (stateless) with value '{}' could not be found",
                                                             value)).chain(item -> item == null ? Uni.createFrom()
                                                                                                     .nullItem() : session.fetch(
                        item.getInvolvedPartyID()));
    }

    @Override
    public Uni<IInvolvedParty<?, ?>> findByToken(Mutiny.StatelessSession session,
                                                 ISecurityToken<?, ?> token,
                                                 UUID... identityToken) {
        ...
        return findInvolvedPartyIdentificationType(session,
                ...
                .chain(item -> session.fetch(item.getInvolvedPartyID())));
    }

    @Override
    public Uni<IInvolvedParty<?, ?>> findByUUID(Mutiny.StatelessSession session,
                                                UUID token,
                                                ISystems<?, ?> system,
                                                UUID... identityToken) {
        ...
        return findInvolvedPartyIdentificationType(session, identTypeName, system, identityToken)
                .chain(id -> new InvolvedPartyXInvolvedPartyIdentificationType().builder(session).findLink(null,
                                                                                ...
                                                                                .withEnterprise(enterprise)
                                                                                .canRead(system, identityToken).get()
                                                                                .onFailure(NoResultException.class)
                                                                                .invoke(e -> log.warn(
                                                                                        "InvolvedParty by UUID (stateless) '{}' could not be found",
                                                                                        token)))
                .chain(idxid -> session.fetch(idxid.getInvolvedPartyID())).invoke(involvedParty -> {
                    if (involvedParty != null && involvedParty.getId() != null) {
    ...
    public Uni<List<IRelationshipValue<IInvolvedParty<?, ?>, IInvolvedPartyIdentificationType<?, ?>, ?>>> findAllByIdentificationType(
            ...
            String value) {
        log.debug("Finding all InvolvedParties by identification type (stateless): {}, value: {}",
                  identificationType,
                  value);
    ...
    @Override
    public Uni<List<IInvolvedParty<?, ?>>> findByRulesClassification(Mutiny.StatelessSession session,
        ...
        return classificationService.find(session, classification, system, identityToken)
                                    .chain(classification1 -> new InvolvedPartyXRules().builder(session)
                                                                                       .withClassification(
                                                                                               classification1)
                                                                                       .withValue(value).inActiveRange()
                                                                                       .inDateRange().getAll()
                                                                                       .chain(list -> {
                                                                                           Uni<List<IInvolvedParty<?, ?>>> acc = Uni
                                                                                                   .createFrom()
                                                                                                   .item(new ArrayList<>());
                                                                                           for (InvolvedPartyXRules item : list) {
                                                                                               acc = acc.chain(result -> session
                                                                                                       .fetch(item.getInvolvedPartyID())
                                                                                                       .map(p -> {
                                                                                                           result.add(p);
                                                                                                           return result;
                                                                                                       }));
    ...
    @Override
    public Uni<IInvolvedParty<?, ?>> findByClassification(Mutiny.StatelessSession session,
                                                          String classification,
                                                          String value,
                                                          ISystems<?, ?> system,
                                                          UUID... identityToken) {
        ...
        return classificationService.find(session, classification, system, identityToken)
                                    .chain(classification1 -> new InvolvedPartyXClassification().builder(session)
                                                                                                ...
                                                                                                .invoke(e -> log.warn(
                                                                                                        "InvolvedParty by classification value {}  (stateless) '{}' could not be found",
                                                                                                        classification,
                                                                                                        value))
                                                                                                .chain(item -> item == null ? Uni
                                                                                                                              .createFrom()
                                                                                                                              .nullItem() : session.fetch(
                                                                                                        item.getPrimary()))
