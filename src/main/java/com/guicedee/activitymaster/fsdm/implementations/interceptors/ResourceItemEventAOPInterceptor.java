...
public class ResourceItemEventAOPInterceptor implements MethodInterceptor
{
	...
	/*	if (!configuration.isEnterpriseReady())
		...
		// Get the resource item event annotation
		ResourceItemEvent resourceItemEvent = methodInvocation.getMethod()
		...
		enterpriseService.getEnterprise(enterpriseName)
			.subscribe().with(
				enterprise -> {
					// Process each resource item in parallel
					for (Pair<ResourceItem, IResourceItem<?, ?>> pair : refObject)
					{
						...
						// Create an operation for each resource item
								...
								return getISystem(ActivityMasterSystemName, enterprise)
									.chain(system -> {
										return getISystemToken(ActivityMasterSystemName, enterprise)
											.chain(token -> {
												// Add resource item to event
												return Uni.createFrom().item(() -> {
													this.event.addResourceItem(classification, pair.getValue(), resourceItemEvent.classificationName(), system, token);
													return null;
												});
											});
									});
					...
					if (!operations.isEmpty()) {
						Uni.combine().all().unis(operations)
							.discardItems()
							.subscribe().with(
								result -> log.info("All resource item operations completed successfully"),
								error -> log.log(Level.SEVERE, "Error in parallel resource item operations", error)
							);
					}
				},
						...
						.chain(token -> {
							...
							return Uni.createFrom().item(() -> {
								try {
									...
									classificationService.create(classificationName, classificationName, EventXResourceItem, system, 0, "LogItemTypes", token);
									return null;
								}
							});
						});
