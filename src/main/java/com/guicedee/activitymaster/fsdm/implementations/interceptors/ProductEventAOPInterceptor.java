...
public class ProductEventAOPInterceptor implements MethodInterceptor
{
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable
	{
		...
		// Get the product event annotation
		ProductEvent productEvent = methodInvocation.getMethod()
			...
			.subscribe().with(
				enterprise -> {
					...
					for (Pair<Product, IProduct<?, ?>> pair : refObject)
					{
						...
						// Create an operation for each product
										...
										return getISystemToken(ActivityMasterSystemName, enterprise)
											.chain(token -> {
												...
												return Uni.createFrom().item(() -> {
													this.event.addProduct(pair.getValue(), classification, productEvent.classificationName(), system, token);
													return null;
												});
											});
									});
					...
					if (!operations.isEmpty()) {
						Uni.combine().all().unis(operations)
							.discardItems()
							.subscribe().with(
								result -> log.info("All product operations completed successfully"),
								error -> log.log(Level.SEVERE, "Error in parallel product operations", error)
							);
					}
				},
						...
						.chain(token -> {
							...
							return Uni.createFrom().item(() -> {
								try {
									...
									classificationService.create(classificationName, classificationName, EventXProduct, system, 0, "LogItemTypes", token);
									return null;
								}
							});
						});
