...
public class EventsAOPInterceptor implements MethodInterceptor
{
    ...
    {
        return eventsByThread.get(Thread.currentThread()
                                          .getId());
    }
    ...
    public static void setCurrentEvent(IEvent<?, ?> event)
    {
        if (event == null)
        {
            eventsByThread.remove(Thread.currentThread()
                                          .getId());
        }
        else
        {
            eventsByThread.put(Thread.currentThread()
                                       .getId(), event);
        }
    }
...
*/
        ...
        eventService.findEventType(eventAnnotation.value(), system, identityToken)
                .chain(eventType -> {
                    return eventService.createEvent(eventAnnotation.value(), system, identityToken)
                                   .chain(event -> {
