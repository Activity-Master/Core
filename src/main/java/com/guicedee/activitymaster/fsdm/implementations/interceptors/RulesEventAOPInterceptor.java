...
public class RulesEventAOPInterceptor implements MethodInterceptor
{
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable
	{
		...
		// Get the rules event annotation
		RulesEvent rulesEvent = methodInvocation.getMethod()
			...
			.subscribe().with(
				enterprise -> {
					...
					for (Pair<RuleSet, IRules<?, ?>> pair : refObject)
					{
						...
						// Create an operation for each rule
										...
										return getISystemToken(ActivityMasterSystemName, enterprise)
											.chain(token -> {
												...
												return Uni.createFrom().item(() -> {
													this.event.addRules(pair.getValue(), classification, rulesEvent.classificationName(), system, token);
													return null;
												});
											});
									});
							})
							.onFailure().invoke(error -> log.log(Level.SEVERE, "Error processing rule: " + classification, error));
					...
					if (!operations.isEmpty()) {
						Uni.combine().all().unis(operations)
							.discardItems()
							.subscribe().with(
								result -> log.info("All rule operations completed successfully"),
								error -> log.log(Level.SEVERE, "Error in parallel rule operations", error)
							);
					}
				},
						...
						.chain(token -> {
							...
							return Uni.createFrom().item(() -> {
								try {
									...
									classificationService.create(classificationName, classificationName, EventXRules, system, 0, "LogItemTypes", token);
									return null;
								}
							});
						});
