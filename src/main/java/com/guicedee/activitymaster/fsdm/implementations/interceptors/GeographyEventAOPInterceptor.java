...
public class GeographyEventAOPInterceptor implements MethodInterceptor
{
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable
	{
		...
		{
			return ((Uni<?>) result).onItem().call(() -> processGeographies(methodInvocation, currentEvent));
		}
		else
		{
			processGeographies(methodInvocation, currentEvent)
				.subscribe().with(
					item -> log.info("Geography processed successfully"),
					error -> log.log(Level.SEVERE, "Error processing geographies", error)
				);
			return result;
		}
	}

	private Uni<Void> processGeographies(MethodInvocation methodInvocation, IEvent<?, ?> currentEvent)
	{
		...
		GeographyEvent geographyEvent = methodInvocation.getMethod().getAnnotation(GeographyEvent.class);
					...
					for (Pair<Geography, IGeography<?, ?>> pair : refObject)
					{
						...
						operations.add(checkClassificationExists(session, classification, enterprise)
							...
							.chain(system -> getISystemToken(session, ActivityMasterSystemName, enterprise)
								.chain(token -> currentEvent.addGeography(session, pair.getValue(), classification, geographyEvent.classificationName(), system, token)))
							.onFailure().invoke(error -> log.log(Level.SEVERE, "Error processing geography: " + classification, error))
						);
					}
			...
			.chain(system -> getISystemToken(session, ActivityMasterSystemName, enterprise)
				.chain(token -> {
					return classificationService.find(session, classificationName, system, token)
						.onFailure().recoverWithUni(() -> classificationService.create(session, classificationName, classificationName, EventXGeography, system, 0, "LogItemTypes", token))
