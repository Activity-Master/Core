...
public class GeographyEventAOPInterceptor implements MethodInterceptor
{
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable
	{
		...
		// Get the geography event annotation
		GeographyEvent geographyEvent = methodInvocation.getMethod()
		...
		enterpriseService.getEnterprise(enterpriseName)
			.subscribe().with(
				enterprise -> {
					// Process each geography in parallel
					for (Pair<Geography, IGeography<?, ?>> pair : refObject)
					{
						...
						// Create an operation for each geography
								...
								return getISystem(ActivityMasterSystemName, enterprise)
									.chain(system -> {
										return getISystemToken(ActivityMasterSystemName, enterprise)
											.chain(token -> {
												// Add geography to event
												return Uni.createFrom().item(() -> {
													this.event.addGeography(pair.getValue(), classification, geographyEvent.classificationName(), system, token);
													return null;
												});
											});
									});
					...
					if (!operations.isEmpty()) {
						Uni.combine().all().unis(operations)
							.discardItems()
							.subscribe().with(
								result -> log.info("All geography operations completed successfully"),
								error -> log.log(Level.SEVERE, "Error in parallel geography operations", error)
							);
					}
				},
						...
						.chain(token -> {
							...
							return Uni.createFrom().item(() -> {
								try {
									...
									classificationService.create(classificationName, classificationName, EventXGeography, system, 0, "LogItemTypes", token);
									return null;
								}
							});
						});
