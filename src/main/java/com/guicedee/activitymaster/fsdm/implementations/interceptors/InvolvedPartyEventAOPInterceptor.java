...
public class InvolvedPartyEventAOPInterceptor implements MethodInterceptor
{
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable
	{
		...
		{
			return ((Uni<?>) result).onItem().call(() -> processInvolvedParties(methodInvocation, currentEvent));
		}
		else
		{
			processInvolvedParties(methodInvocation, currentEvent)
				.subscribe().with(
					item -> log.info("Involved party processed successfully"),
					error -> log.log(Level.SEVERE, "Error processing involved parties", error)
				);
			return result;
		}
	}

	private Uni<Void> processInvolvedParties(MethodInvocation methodInvocation, IEvent<?, ?> currentEvent)
	{
		...
		InvolvedPartyEvent involvedPartyEvent = methodInvocation.getMethod().getAnnotation(InvolvedPartyEvent.class);
					...
					for (Pair<Party, IInvolvedParty<?, ?>> pair : refObject)
					{
						...
						operations.add(checkClassificationExists(session, classification, enterprise)
							...
							.chain(system -> getISystemToken(session, ActivityMasterSystemName, enterprise)
								.chain(token -> currentEvent.addInvolvedParty(session, pair.getValue(), classification, involvedPartyEvent.classificationName(), system, token)))
							.onFailure().invoke(error -> log.log(Level.SEVERE, "Error processing involved party: " + classification, error))
						);
					}
			...
			.chain(system -> getISystemToken(session, ActivityMasterSystemName, enterprise)
				.chain(token -> {
					return classificationService.find(session, classificationName, system, token)
						.onFailure().recoverWithUni(() -> classificationService.create(session, classificationName, classificationName, EventXInvolvedParty, system, 0, "LogItemTypes", token))
