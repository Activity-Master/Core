...
public class ClassificationEventAOPInterceptor implements MethodInterceptor
{
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable
	{
		...
		// Get the classification event annotation
		ClassificationEvent classificationEvent = methodInvocation.getMethod()
		...
		enterpriseService.getEnterprise(enterpriseName)
			.subscribe().with(
				enterprise -> {
					// Process each classification in parallel
					for (Pair<Classification, IClassification<?, ?>> pair : refObject)
					{
						...
						String classification = pair.getValue().getName();

						// Create an operation for each classification
										...
										return getISystemToken(ActivityMasterSystemName, enterprise)
											.chain(token -> {
												...
												return Uni.createFrom().item(() -> {
													this.event.addClassification(classification, EventXClassification, classificationEvent.classificationName(), system, token);
													return null;
												});
											});
									});
							})
							.onFailure().invoke(error -> log.log(Level.SEVERE, "Error processing classification: " + classification, error));
					...
					if (!operations.isEmpty()) {
						Uni.combine().all().unis(operations)
							.discardItems()
							.subscribe().with(
								result -> log.info("All classification operations completed successfully"),
								error -> log.log(Level.SEVERE, "Error in parallel classification operations", error)
							);
					}
				},
						...
						.chain(token -> {
							...
							return Uni.createFrom().item(() -> {
								try {
									...
									classificationService.create(classificationName, classificationName, EventXClassification, system, 0, "LogItemTypes", token);
									return null;
								}
							});
						});
