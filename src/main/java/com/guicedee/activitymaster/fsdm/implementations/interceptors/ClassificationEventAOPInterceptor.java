...
public class ClassificationEventAOPInterceptor implements MethodInterceptor
{
	...
	public Object invoke(MethodInvocation methodInvocation) throws Throwable
	{
		...
		{
			return ((Uni<?>) result).onItem().call(() -> processClassifications(methodInvocation, currentEvent));
		}
		else
		{
			processClassifications(methodInvocation, currentEvent)
				.subscribe().with(
					item -> log.info("Classification processed successfully"),
					error -> log.log(Level.SEVERE, "Error processing classifications", error)
				);
			return result;
		}
	}

	private Uni<Void> processClassifications(MethodInvocation methodInvocation, IEvent<?, ?> currentEvent)
	{
		...
		ClassificationEvent classificationEvent = methodInvocation.getMethod().getAnnotation(ClassificationEvent.class);
			...
			return enterpriseService.getEnterprise(session, enterpriseName)
				.chain(enterprise -> {
					...
					for (Pair<Classification, IClassification<?, ?>> pair : refObject)
					{
						...
						String classificationName = pair.getValue().getName();
						
						operations.add(checkClassificationExists(session, classificationName, enterprise)
							...
							.chain(system -> getISystemToken(session, ActivityMasterSystemName, enterprise)
								.chain(token -> currentEvent.addClassification(session, classificationName, EventXClassification, classificationEvent.classificationName(), system, token)))
							.onFailure().invoke(error -> log.log(Level.SEVERE, "Error processing classification: " + classificationName, error))
						);
					}
			...
			.chain(system -> getISystemToken(session, ActivityMasterSystemName, enterprise)
				.chain(token -> {
					return classificationService.find(session, classificationName, system, token)
						.onFailure().recoverWithUni(() -> classificationService.create(session, classificationName, classificationName, EventXClassification, system, 0, "LogItemTypes", token))
