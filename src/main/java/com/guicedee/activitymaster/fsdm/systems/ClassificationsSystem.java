...
public class ClassificationsSystem
                ...
                .chain(iSystems -> {
                    ...
                    return getSystem(session, enterprise)
                            ...
                            .chain(() -> Uni.createFrom()
                                    .item(iSystems)); // Chain back to return the original system
                });
    }
    ...
    private Uni<Void> createEnterpriseRootClassification(Mutiny.Session session, IEnterprise<?, ?> enterprise, ISystems<?, ?> system) {
        ...
        return service.create(session, enterprise.getName(), enterprise.getName(),
                ...
                .onFailure()
                .invoke(error -> log.error("❌ Failed to create enterprise root classification: {}",
                        error.getMessage(), error))
                .chain(v -> {
    ...
    private Uni<Void> createBaseClassifications(Mutiny.Session session, IEnterprise<?, ?> enterprise, ISystems<?, ?> system) {
        ...
        return service.create(session, DefaultClassifications.HierarchyTypeClassification, system)
                .chain(v -> {
                    log.debug("📋 Creating NoClassification");
                    return service.create(session, DefaultClassifications.NoClassification, system, enterprise.getName());
                })
                .chain(v -> {
                    log.debug("📋 Creating DefaultClassification");
                    return service.create(session, DefaultClassifications.DefaultClassification, system, enterprise.getName());
                })
                .chain(v -> {
    ...
    private Uni<Void> createSecurityClassifications(Mutiny.Session session, IEnterprise<?, ?> enterprise, ISystems<?, ?> system) {
        ...
        return service.create(session, DefaultClassifications.Security, system, enterprise.getName())
                ...
                .onFailure()
                .invoke(error -> log.error("❌ Failed to create Security classification: {}",
                        error.getMessage(), error))
                .flatMap(securityClassification -> {
                    ...
                    return service.create(session, SystemsClassifications.SystemIdentity, system, DefaultClassifications.Security)
                            .onItem()
                            .invoke(result -> log.debug("✅ Created SystemIdentity classification with ID: {}", result.getId()))
                            .onFailure()
                            .invoke(error -> log.error("❌ Failed to create SystemIdentity classification: {}",
                                    error.getMessage(), error));
                })
                .flatMap(v -> {
                    ...
                    return service.create(session, InvolvedPartyClassifications.SecurityPassword, system, DefaultClassifications.Security)
                            .onItem()
                            .invoke(result -> log.debug("✅ Created SecurityPassword classification with ID: {}", result.getId()))
                            .onFailure()
                            .invoke(error -> log.error("❌ Failed to create SecurityPassword classification: {}",
                                    error.getMessage(), error));
                })
                .flatMap(v -> {
                    ...
                    return service.create(session, InvolvedPartyClassifications.SecurityPasswordSalt, system, DefaultClassifications.Security)
                            .onItem()
                            .invoke(result -> log.debug("✅ Created SecurityPasswordSalt classification with ID: {}", result.getId()))
                            .onFailure()
                            .invoke(error -> log.error("❌ Failed to create SecurityPasswordSalt classification: {}",
                                    error.getMessage(), error));
                })
                .chain(v -> {
    ...
    private Uni<Void> createEnterpriseClassifications(Mutiny.Session session, IEnterprise<?, ?> enterprise, ISystems<?, ?> system) {
        ...
        return service.create(session, EnterpriseClassifications.LastUpdateDate, system, enterprise.getName())
                .onItem()
                .invoke(result -> log.debug("✅ Created LastUpdateDate classification with ID: {}", result.getId()))
                .onFailure()
                .invoke(error -> log.error("❌ Failed to create LastUpdateDate classification: {}",
                        error.getMessage(), error))
                .chain(v -> {
                    log.debug("📋 Creating UpdateClass classification");
                    return service.create(session, EnterpriseClassifications.UpdateClass, system, enterprise.getName());
                })
                .onItem()
                .invoke(result -> log.debug("✅ Created UpdateClass classification with ID: {}", result.getId()))
                .onFailure()
                .invoke(error -> log.error("❌ Failed to create UpdateClass classification: {}",
                        error.getMessage(), error))
                .chain(v -> {
                    log.debug("📋 Creating EnterpriseIdentity classification");
                    return service.create(session, EnterpriseClassifications.EnterpriseIdentity, system, enterprise.getName());
                })
                .onItem()
                .invoke(result -> {
                ...
                .onFailure()
                .invoke(error -> log.error("❌ Failed to create EnterpriseIdentity classification: {}",
                        error.getMessage(), error))
                .replaceWithVoid();
    }
