        ...
        implements IActivityMasterSystem<InvolvedPartySystem>
{
    ...
    public Uni<Void> createDefaults(Mutiny.Session session, IEnterprise<?, ?> enterprise)
    {
        ...
        return sessionFactory.withTransaction((newSession, tx) -> {
            log.info("🔄 Created transaction with session: {}", newSession.hashCode());
            
            // Chain the operations in sequence using the same session
            return createIdentificationTypes(newSession, enterprise)
                    .flatMap(v -> {
                        log.debug("🔄 Identification types created, proceeding to name types");
                        return createNameTypes(newSession, enterprise);
                    })
                    .flatMap(v -> {
                        log.debug("🔄 Name types created, proceeding to involved party types");
                        return createTypes(newSession, enterprise);
                    })
                    .onFailure()
