 ...
 */
@Log4j2
public class TimeSystem extends MasterDefaultSystem<TimeSystem> implements IMasterSystem<TimeSystem>, ITimeSystem, IProgressable {
    ...
    public Uni<ISystems<?, ?>> registerSystem(Mutiny.StatelessSession session, IEnterprise<?, ?> enterprise) {
        ...
        return systemsService.doesSystemExist(session, enterprise, getSystemName()).chain(exists -> {
            if (!exists) {
                return systemsService.create(session, enterprise, getSystemName(), getSystemDescription()).onFailure()
                                     .call((err) -> {
                            ...
                            log.debug("✅ Found/Created Time System: '{}' with session: {}",
                                      system.getName(),
                                      session.hashCode());

                            // Chain the registerNewSystem call properly
                            return systemsService.registerNewSystem(session, enterprise, system).onItem().invoke(() -> {
                                log.debug("✅ Registered system: {}", getSystemName());
                                log.info("🎉 Successfully registered Time System for enterprise: '{}'",
                                         enterprise.getName());
                            }).onFailure().invoke(error -> log.error("❌ Error registering system: {}",
                                                                     error.getMessage(),
                                                                     error)).replaceWith(system); // Return the system
                        }).onFailure().invoke(error -> log.error(
                                "❌ Failed to create Time System: '{}' with session {}: {}",
                                getSystemName(),
                                session.hashCode(),
                                error.getMessage(),
                                error));
            } else {
                return systemsService.findSystem(session, enterprise, getSystemName(), null);
            }
        });
    }
    ...
    public Uni<Void> loadTimeRange(int startYear, int endYear) {
        ...
        return Uni.createFrom().item(() -> {
            ...
            while (startYearGC.getTime().getTime() <= endGC.getTime()
                                                           .getTime() && (processedCount < maxDaysToProcess || totalDays <= maxDaysToProcess)) {
                ...
                chain = chain.chain(() -> {
                    ...
                    return SessionUtils.withActivityMasterStateless(ActivityMasterConfiguration.applicationEnterpriseName,
                                                                    TimeSystemName,
                                                                    tuple -> {
                                                                        Mutiny.StatelessSession session = tuple.getItem1();
                                                                        return getDay(session, currentDate).onItem()
                                                                                                           .invoke(day -> {
    ...
    private Uni<Void> processDaysInBatches(List<Uni<Days>> dayOperations, int batchSize) {
        ...
        for (int i = 0; i < dayOperations.size(); i += batchSize) {
            ...
            chain = chain.chain(() -> {
                ...
                Uni<Void> batchChain = Uni.createFrom().voidItem();

                for (Uni<Days> dayOp : batch) {
                    batchChain = batchChain.chain(dayOp::replaceWithVoid);
                }

                return batchChain.onItem().invoke(() -> {
    ...
    public Uni<Years> getYear(Mutiny.StatelessSession session, Date date) {
        ...
        return getYearFromDatabase(session, date).onItem().invoke(year -> {
            log.debug("✅ Retrieved year for date: {}", date);
        }).onFailure().recoverWithUni(error -> {
            log.debug("📋 Year not found for date: {}, creating new year", date);
            return loadTimeRange(date.getYear(), date.getYear()).chain(a -> {
    ...
    private Uni<Years> getYearFromDatabase(Mutiny.StatelessSession session, Date date) {
        Short yearId = Short.parseShort(YearIDFormat.getSimpleDateFormat().format(date));
        return new Years().builder(session).find(yearId).get().onFailure(NoResultException.class)
                          .recoverWithUni(createYear(session, date));
    }
     ...
     */
    private Uni<Years> createYear(Mutiny.StatelessSession session, Date date) {
        Short yearId = Short.parseShort(YearIDFormat.getSimpleDateFormat().format(date));
        ...
        // Get last year ID
        return getLastYearID(date).chain(lastYearId -> {
            ...
            // Insert the year using the stateless session
            return session.insert(year).chain(() -> Uni.createFrom().item(year));
        });
    }
    ...
    public Uni<Quarters> getQuarter(Mutiny.StatelessSession session, Date date) {
        ...
        return getQuarterFromDatabase(session, date).onItem().invoke(quarter -> {
    ...
    private Uni<Quarters> getQuarterFromDatabase(Mutiny.StatelessSession session, Date date) {
        ...
        return new Quarters().builder(session).find(quarterId).get().onItem().transform(optional -> {
            if (optional != null) {
    ...
    private Uni<Quarters> createQuarter(Mutiny.StatelessSession session, Date date) {
        ...
        return getLastQuarterID(date).chain(lastQuarterId -> {
            ...
            return getLastYearQuarterID(date).chain(lastYearQuarterId -> {
                log.debug("✅ Got last year quarter ID: {}", lastYearQuarterId);
                quarter.setLastYearID(lastYearQuarterId.shortValue());

                log.debug("📋 Getting year for date: {}", date);
                return getYear(session, date).chain(year -> {
                    log.debug("✅ Got year: {}", year.getId());
                    quarter.setYearID(year);

                    // Get quarter number
                    log.debug("📋 Getting quarter number for date: {}", date);
                    return getQuarterNumber(date).chain(quarterNumber -> {
                        log.debug("✅ Got quarter number: {}", quarterNumber);

                        // Set quarter descriptions
                        quarter.setQuarterInYear(quarterNumber);
                        quarter.setQuarterDescription("Q" + quarterNumber + " " + YearIDFormat.getSimpleDateFormat()
                                                                                              .format(date));
                        quarter.setQuarterGraphDescription("Q" + quarterNumber + " - " + YearIDFormat
                                .getSimpleDateFormat().format(date));
                        quarter.setQuarterGridDescription("Quarter " + quarterNumber + " - " + YearIDFormat
                                .getSimpleDateFormat().format(date));
                        quarter.setQuarterQQMMDescription("Q" + String.format("%02d",
                                                                              quarterNumber) + " " + MonthNumberFormat
                                .getSimpleDateFormat().format(date));
                        quarter.setQuarterSmallDescription("Quart " + quarterNumber + " " + YearIDFormat
                                .getSimpleDateFormat().format(date));
                        quarter.setQuarterYYMMDescription("'" + YearShortFormat.getSimpleDateFormat()
                                                                               .format(date) + " Q" + String.format(
                                "%02d",
                                quarterNumber));
    ...
    private int getQuarterID(Date date) {
        GregorianCalendar gc = new GregorianCalendar();
        gc.setTime(date);
        return Integer.parseInt(YearIDFormat.getSimpleDateFormat().format(date) + getQuarterNumber(gc));
    }
    ...
    public Uni<Months> getMonth(Mutiny.StatelessSession session, Date date) {
        ...
        return getMonthFromDatabase(session, date).onItem().invoke(month -> {
    ...
    private Uni<Months> getMonthFromDatabase(Mutiny.StatelessSession session, Date date) {
        int monthId = Integer.parseInt(MonthIDFormat.getSimpleDateFormat().format(date));
        return new Months().builder(session).find(monthId).get().onItem().transform(optional -> {
            if (optional != null) {
    ...
    private Uni<Months> createMonth(Mutiny.StatelessSession session, Date date) {
        ...
        return getLastMonthID(date).chain(lastMonthId -> {
    ...
    private Uni<MonthOfYear> getMonthOfYear(Mutiny.StatelessSession session, Integer monthNumber) {
        return new MonthOfYear().builder(session).find(monthNumber).get().onItem().transform(optional -> {
            if (optional != null) {
    ...
    public Uni<Weeks> getWeek(Mutiny.StatelessSession session, Date date) {
        ...
        return getWeekFromDatabase(session, date).onItem().invoke(week -> {
    ...
    private Uni<Weeks> getWeekFromDatabase(Mutiny.StatelessSession session, Date date) {
        ...
        return new Weeks().builder(session).find(weekId).get().onItem().transform(optional -> {
            if (optional != null) {
    ...
    private Uni<Weeks> createWeek(Mutiny.StatelessSession session, Date date) {
        ...
        // Get related entities and IDs sequentially using chains
        log.debug("📋 Getting month for date: {}", date);
        return getMonth(session, date).chain(month -> {
    ...
    public Uni<Days> getDay(Mutiny.StatelessSession session, Date date) {
        ...
        return getDayFromDatabase(session, date).onItem().invoke(day -> {
            log.debug("✅ Retrieved day for date: {}", date);
        }).onFailure().recoverWithUni(error -> {
            log.debug("📋 Day not found for date: {}, creating new day", date);
            return createDay(session, date).chain(days -> {
                return populateTransformationTables(session, date, -3).onFailure().invoke(t -> log.error(
                        "Failed to populate transformation tables for day - {}",
                        date,
                        t)).replaceWith(Uni.createFrom().item(days));
            });
        });
    }
    ...
    private Uni<Days> getDayFromDatabase(Mutiny.StatelessSession session, Date date) {
        ...
        return new Days().builder(session).find(dayId).get().onItem().transform(optional -> {
            if (optional != null) {
    ...
    private Uni<Days> createDay(Mutiny.StatelessSession session, Date date) {
        ...
        // Get related entities and IDs sequentially using chains
        log.debug("📋 Getting week for date: {}", date);
        return getWeek(session, date).chain(week -> {
            ...
            return getMonth(session, date).chain(month -> {
                ...
                return getQuarter(session, date).chain(quarter -> {
                    ...
                    return getYear(session, date).chain(year -> {
                        ...
                        return getDayNameEntity(session, date).chain(dayName -> {
                            ...
                            day.setLastDayID(Integer.parseInt(DayIDFormat.getSimpleDateFormat().format(prevDay)));
                            day.setLastMonthID(Integer.parseInt(MonthIDFormat.getSimpleDateFormat().format(prevDay)));
                            day.setLastQuarterID(getQuarterID(prevDay));
                            day.setLastYearID(Integer.parseInt(YearIDFormat.getSimpleDateFormat().format(prevDay)));

                            // Set day descriptions
                            String dayMonthDesc = DayMonthDescriptionFormat.getSimpleDateFormat().format(date);
                            day.setDayMonthDescription(dayMonthDesc);
                            day.setDayDDMMYYYYDescription(DayDDMMYYYYFormat.getSimpleDateFormat().format(date));
                            ...
                            day.setDayDDMMYYYYHyphenDescription(DayHyphenDDMMYYYYFormat.getSimpleDateFormat()
                                                                                       .format(date));
                            day.setDayLongDescription(DayLongDescriptionFormat.getSimpleDateFormat().format(date));
                            day.setDayFullDescription(DayFullDescriptionFormat.getSimpleDateFormat().format(date));

                            // Set additional required descriptions
                            String mmQQDesc = String.format("%02d", gc.get(Calendar.MONTH) + 1) + "Q" + String.format(
                                    "%d",
                                    getQuarterNumber(gc));
                            day.setDayMMQQDescription(mmQQDesc);

                            String yyyyMMDesc = String.format("%04d", gc.get(Calendar.YEAR)) + String.format("%02d",
    ...
    private Uni<DayNames> getDayNameEntity(Mutiny.StatelessSession session, String dayName) {
        return new DayNames().builder(session).where(DayNames_.dayName, Equals, dayName).get().onItem()
                             .transform(optional -> {
                                 if (optional != null) {
                             ...
                             }).onFailure().recoverWithUni(() -> createDayNameEntity(session, dayName));
    }
    ...
    private Uni<DayNames> createDayNameEntity(Mutiny.StatelessSession session, String dayName) {
        ...
        return session.insert(newDayName).chain(() -> Uni.createFrom().item(newDayName));
    }
    ...
    public Uni<Void> createTime() {
        ...
        return SessionUtils.withActivityMasterStateless(ActivityMasterConfiguration.applicationEnterpriseName,
                                                        ...
                                                        tuple -> {
                                                            Mutiny.StatelessSession session = tuple.getItem1();
                                                            return new Hours().builder(session)
                                                                              .where(Hours_.id, Equals, 1).get()
                                                                        ...
                                                                        return Uni.createFrom().voidItem();
                                                                              ...
                                                                              .recoverWithUni(() -> {
                                                                                  ...
                                                                                  log.info(
                                                                                          "🚀 Creating time entities (hours, minutes, half-hours)");
                                                                                  final AtomicInteger dayPartCount = new AtomicInteger(
                                                                                          0);
                                                                                  ...
                                                                                  for (int hr = 0; hr < 24; hr++) {
                                                                                      ...
                                                                                      hourEntity.setTwelveHour(hour > 12 ? "" + String.format(
                                                                                              "%02d",
                                                                                      ...
                                                                                      for (int min = 0; min < 60; min += 15) {
                                                                                          ...
                                                                                          // Create time entity
                                                                                          TimePK timePK = new TimePK(
                                                                                                  hour,
                                                                                                  minute);
                                                                                          Time time = new Time(timePK);
                                                                                          time.setAmPmDesc(hour < 13 ? "AM" : "PM");
                                                                                          time.setTwelveHoursDesc(hour > 12 ? "" + String.format(
                                                                                                  "%02d",
                                                                                                  hour - 12) + ":" + String.format(
                                                                                                  "%02d",
                                                                                                  ...
                                                                                                  "%02d",
                                                                                                  minute));
                                                                                          time.setTwentyFourHoursDesc(
                                                                                                  String.format("%02d",
                                                                                                                hour) + ":" + String.format(
                                                                                                          "%02d",
                                                                                                          minute));
                                                                                          time.setPreviousHourID(hour == 0 ? 23 : hour - 1);
                                                                                          time.setPreviousMinuteID(
                                                                                          ...
                                                                                          if (minute == 0 || minute == 30) {
                                                                                              HalfHours halfHour = new HalfHours(
                                                                                                      new TimePK(hour,
                                                                                                                 minute));
                                                                                              halfHour.setAmPmDesc(hour < 13 ? "AM" : "PM");
                                                                                              halfHour.setTwelveHourClockDesc(
                                                                                                      hour > 12 ? "" + String.format(
                                                                                                              "%02d",
                                                                                                              hour - 12) + ":" + String.format(
                                                                                                              "%02d",
                                                                                                              ...
                                                                                                              "%02d",
                                                                                                              minute));
                                                                                              halfHour.setTwentyFourHourClockDesc(
                                                                                                      String.format(
                                                                                                              "%02d",
                                                                                                              hour) + ":" + String.format(
                                                                                                              "%02d",
                                                                                                              minute));
                                                                                              halfHour.setPreviousHourID(
                                                                                                      hour == 0 ? 23 : hour - 1);
                                                                                              halfHour.setPreviousHalfHourMinuteID(
                                                                                                      minute == 0 ? 30 : 0);
                                                                                              ...
                                                                                              chain = chain.chain(() -> {
                                                                                                  ...
                                                                                                  return session
                                                                                                          ...
                                                                                                          .chain(() -> {
                                                                                                              ...
                                                                                                              halfHourDayParts.setId(
                                                                                                                      currentDayPartCount);
                                                                                                              halfHourDayParts.setHourID(
                                                                                                                      finalHalfHour
                                                                                                                              .getId()
                                                                                                                              .getHourID());
                                                                                                              halfHourDayParts.setMinuteID(
                                                                                                                      finalHalfHour
                                                                                                                              .getId()
                                                                                                                              .getMinuteID());

                                                                                                              // Get DayPart from TimeService
                                                                                                              int dayPartId = getDayPartId(
                                                                                                                      finalHalfHour
                                                                                                                              .getId()
                                                                                                                              .getHourID(),
                                                                                                                      finalHalfHour
                                                                                                              ...
                                                                                                              return session
                                                                                                                      ...
                                                                                                                      .chain(dayPart -> {
                                                                                                                          if (dayPart == null) {
                                                                                                                              ...
                                                                                                                              newDayPart.setDayPartName(
                                                                                                                                      dayPartName);
                                                                                                                              newDayPart.setDayPartDescription(
                                                                                                                                      dayPartDesc);
                                                                                                                              newDayPart.setDayPartSortOrder(
                                                                                                                                      dayPartId);
                                                                                                                              ...
                                                                                                                              return session
                                                                                                                                      ...
                                                                                                                                      .chain(() -> {
                                                                                                                                          log.info(
                                                                                                                                                  "✅ Created DayPart: " + dayPartId);
                                                                                                                                          halfHourDayParts.setDayPartID(
                                                                                                                                                  newDayPart);
                                                                                                                                          log.info(
                                                                                                                                                  "📋 Creating HalfHourDayParts: {}",
                                                                                                                                                  halfHourDayParts.getId());
                                                                                                                                          return session
                                                                                                                                                  .insert(halfHourDayParts)
                                                                                                                                                  .onItem()
                                                                                                                                                  .invoke(() -> {
                                                                                                                                                      log.info(
                                                                                                                                                              "✅ Created HalfHourDayParts: {}",
                                                                                                                                                              halfHourDayParts.getId());
                                                                                                                                                  })
                                                                                                                                                  .onFailure()
                                                                                                                                                  .invoke(e -> {
                                                                                                                                                      log.error(
                                                                                                                                                              "❌ Failed to create HalfHourDayParts {}: {}",
                                                                                                                                                              halfHourDayParts.getId(),
                                                                                                                                                              e.getMessage(),
                                                                                                                                                              e);
                                                                                                                                                  });
                                                                                                                              ...
                                                                                                                              halfHourDayParts.setDayPartID(
                                                                                                                                      dayPart);
                                                                                                                              log.info(
                                                                                                                                      "📋 Creating HalfHourDayParts with existing DayPart: {}",
                                                                                                                                      halfHourDayParts.getId());
                                                                                                                              return session
                                                                                                                                      .insert(halfHourDayParts)
                                                                                                                                      .onItem()
                                                                                                          ...
                                                                                                          .invoke(() -> {
                                                                                                              log.info(
                                                                                                                      "✅ Created half-hour: " + finalHalfHour.getId());
                                                                                                          }).onFailure()
                                                                                                          .invoke(e -> {
                                                                                                              log.error(
                                                                                                                      "❌ Failed to create half-hour {}: {}",
                                                                                                                      finalHalfHour.getId(),
                                                                                                                      e.getMessage(),
                                                                                                                      e);
                                                                                                          });
                                                                                  ...
                                                                                  chain = chain.chain(() -> {
                                                                                      log.info(
                                                                                              "🔍 Collecting created hours and half-hours for logging");
                                                                                      return session.createQuery(
                                                                                                            "SELECT h FROM Hours h ORDER BY h.id",
                                                                                                            Hours.class)
                                                                                                    .getResultList()
                                                                                                    .onItem()
                                                                                                    .invoke(hours -> {
                                                                                                    ...
                                                                                                    .chain(() -> session
                                                                                                            .createQuery(
                                                                                                                    "SELECT h FROM HalfHours h ORDER BY h.id.hourID, h.id.minuteID",
                                                                                                                    HalfHours.class)
                                                                                                            .getResultList()
                                                                                                            .onItem()
                                                                                                            .invoke(halfHours -> {
                                                                                                                log.info(
                                                                                                                        "📊 Collected {} half-hours",
                                                                                                                        halfHours.size());
                                                                                                                log.info(
                                                                                                                        "📋 Created Half-Hours Summary:");
                                                                                  ...
                                                                                  return chain.onItem().invoke(() -> {
                                                                                      log.info(
                                                                                              "✅ Time entities creation completed");
                                                                                  }).onFailure().invoke(e -> {
                                                                                      log.error(
                                                                                              "❌ Failed to create time entities: {}",
                                                                                              e.getMessage(),
                                                                                              e);
                                                                                  });
                                                                              }).onFailure().invoke(e -> {
                                                                        log.error("❌ Failed in createTime: {}",
                                                                                  e.getMessage(),
                                                                                  e);
                                                                    });
                                                        });
    ...
    public Uni<Void> populateTransformationTables(Mutiny.StatelessSession session, Date date, int fiscalLag) {
        ...
        return getDayYTD(session, date)
                .chain(ytdEntities -> getDayQTD(session, date).chain(qtdEntities -> getDayMTD(session, date).chain(
                        mtdEntities -> getDayFiscal(session, date, fiscalLag).chain(fiscalEntities -> {
                            ...
                            log.info("Inserting {} total relationships using stateless session (sequential insert)",
                                     allEntities.size());

                            Uni<Void> insertChain = Uni.createFrom().voidItem();
                            for (Object entity : allEntities) {
                                insertChain = insertChain.chain(() -> session.insert(entity));
                            }
                            return insertChain.replaceWithVoid();
                        })))).onItem().invoke(() -> log.info("Transformation tables populated for date: {}", date))
                .onFailure().invoke(error -> log.error("❌ Failed to populate transformation tables: {}",
                                                       error.getMessage(),
                                                       error));
    }
    ...
    private Uni<List<TransYtd>> getDayYTD(Mutiny.StatelessSession session, Date date) {
        ...
        while (startYearGC.getTime().getTime() <= gc.getTime().getTime()) {
            final Date currentDate = startYearGC.getTime();
            final int ytdDayId = Integer.parseInt(DayIDFormat.getSimpleDateFormat().format(currentDate));
            totalDays++;

            // Create TransYtd entity
            final TransYtd transYtd = new TransYtd().setId(new TransYtdPK().setDayID(dayId).setYtdDayID(ytdDayId));

            // Add to list
            ytdEntities.add(transYtd);
            log.debug("📋 Created YTD relationship: {} -> {}",
                      transYtd.getId().getDayID(),
                      transYtd.getId().getYtdDayID());
        ...
        return Uni.createFrom().item(ytdEntities).onItem().invoke(() -> {
    ...
    private Uni<List<TransQtd>> getDayQTD(Mutiny.StatelessSession session, Date date) {
        ...
        while (startQuarterGC.getTime().getTime() <= gc.getTime().getTime()) {
            ...
            final int qtdDayId = Integer.parseInt(DayIDFormat.getSimpleDateFormat().format(currentDate));
            totalDays++;

            // Create TransQtd entity
            final TransQtd transQtd = new TransQtd().setId(new TransQtdPK().setDayID(dayId).setQtdDayID(qtdDayId));

            // Add to list
            qtdEntities.add(transQtd);
            log.debug("📋 Created QTD relationship: {} -> {}",
                      transQtd.getId().getDayID(),
                      transQtd.getId().getQtdDayID());
        ...
        // Return the list of entities
        return Uni.createFrom().item(qtdEntities).onItem().invoke(() -> {
    ...
    private Uni<List<TransMtd>> getDayMTD(Mutiny.StatelessSession session, Date date) {
        ...
        log.debug("📋 Preparing MTD relationships for day ID: {} (Month: {}/{})",
                  dayId,
                  (startMonthGC.get(Calendar.MONTH) + 1),
                  startMonthGC.get(Calendar.YEAR));
        ...
        while (startMonthGC.getTime().getTime() <= gc.getTime().getTime()) {
            ...
            final int mtdDayId = Integer.parseInt(DayIDFormat.getSimpleDateFormat().format(currentDate));
            totalDays++;

            // Create TransMtd entity
            final TransMtd transMtd = new TransMtd().setId(new TransMtdPK().setDayID(dayId).setMtdDayID(mtdDayId));

            // Add to list
            mtdEntities.add(transMtd);
            log.debug("📋 Created MTD relationship: {} -> {}",
                      transMtd.getId().getDayID(),
                      transMtd.getId().getMtdDayID());
        ...
        // Return the list of entities
        return Uni.createFrom().item(mtdEntities).onItem().invoke(() -> {
    ...
    private Uni<List<TransFiscal>> getDayFiscal(Mutiny.StatelessSession session, Date date, int fiscalLag) {
        ...
        return Uni.createFrom().item(fiscalEntities).onItem().invoke(() -> {
